...
function patchBrowserUse() {
  const appMain = findAsset(/^app-main-.*\.js$/);
  const browserUseAvailability = findAsset(/^use-in-app-browser-use-availability-.*\.js$/);
  const mainProcess = findBuildAsset(/^main-.*\.js$/);
...
function patchExperimentalAppSnapshot() {
  const mainProcess = findBuildAsset(/^main-.*\.js$/);
  const annotationEditor = findAsset(/^annotation-comment-editor-card-.*\.js$/);
  const composer = findAsset(/^composer-.*\.js$/);
